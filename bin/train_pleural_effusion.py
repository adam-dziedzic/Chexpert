...
parser.add_argument(f'--save_path_{timestamp}', default='./save',
                    metavar='SAVE_PATH',
                    type=str,
...
def get_loss(output, target, index, device, cfg):
    if cfg.criterion == 'BCE':
        ...
        acc = (target == output).float().sum() / len(target)
    else:
...
def run(args):
    ...
    dataloader_train = DataLoader(
        PleuralEffusionDataset(in_csv_path=cfg.train_csv, cfg=cfg,
                               mode='train'),
        batch_size=cfg.train_batch_size, num_workers=args.num_workers,
        drop_last=True, shuffle=True)
    dataloader_dev = DataLoader(
        PleuralEffusionDataset(cfg.dev_csv, cfg, mode='dev'),
